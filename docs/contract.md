...
the workspace files stay as curated, and an export without the flag puts the
plain caption back. Either way the trainer re-encodes (`make preprocess-te`).
...
|---|---|---|---|
| `image_dataset/**/{stem}.txt` — caption master | hand-written; **Export is its only writer** (the `master` row, publishing the `workspace/master/` overlay a GUI edit wrote). No stage writes it | the caption mirror (`correct_captions --src`), read only for an image with no revised caption yet; caption index | Caption grammar (§3). Parse via `position_clauses.parse_caption`, not `split(",")`. |
| `post_image_dataset/resized/**/{stem}.txt` — revised caption | every caption stage: `correct_captions` corrects it in place (mirroring the master for an image that has none), autotag / position / audit rewrite it, Export publishes `workspace/resized/` here | `preprocess-te` (`library/preprocess/text.py`, beside the resized image) — this is the caption training encodes | Caption grammar (§3). **Revised first, master as read-only fallback** for every reader on either side; once an image has a revised caption a hand-edit of its master no longer reaches it (edit the revised one, or delete it to re-mirror). Every write pushes the replaced text onto `{stem}.history.txt`. With `--combine_ocr` the published copy (and each variant line) ends in the OCR text clauses (speech, then SFX, each kind deduplicated — speech to one per line said verbatim, SFX to one per sound; lines under `--ocr_min_det` 0.5 or with glyphs under `--ocr_min_glyph` 16 px stay in the sidecar only); `workspace/ocr/**/{stem}.ocr.txt` itself is curation-private. |
