...
- **`gui/`** (torch-free — `test_boundary` runs `create_app()` and asserts no `torch` in `sys.modules`): the `anime-tools-gui` web panel. `stages.py` turns each stage CLI's argparse into a form schema (collected in a *child* interpreter, so a stage's imports can never leak into the server; every stage CLI defers torch/smp/albumentations into its functions, so the dump is ~0.2s and torch-free) and back into an argv — fields listed in `ROOT_FIELDS` (`--src`/`--dst`/`--image-dir`/…) are bound to the dataset roots, hidden from the form and filled by `build_argv(roots=…)`, so the roots are set once in Settings; `jobs.py` runs one `python -m` subprocess at a time and streams it over SSE; `dataset.py` joins the three trees (`src`/`dst`/`masks`, keyed by the same relative path) into the sidebar's image→caption tree, reads/writes single captions, and renders thumbnails. **Only `master` and `derived` are writable** — `.variants.txt` is generated. **The browser never splits a caption**: clause structure comes from `parse_caption` via `/api/dataset/item` and `/api/dataset/parse`. Frontend source is `frontend/` (Solid + TS, bundled by `bun` into the committed `gui/static/index.html`; `make frontend`, CI fails on drift). Settings also lists `downloads.catalog()` with a Download button per model (`/api/models`, `/api/models/download`).
