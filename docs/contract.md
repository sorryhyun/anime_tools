...
|---|---|---|---|
| `image_dataset/**/{stem}.txt` — caption master | hand-written; Export and the GUI caption editor are its only writers | `preprocess-te` (`library/preprocess/text.py`), caption index | Caption grammar (§3). Parse via `position_clauses.parse_caption`, not `split(",")`. |
