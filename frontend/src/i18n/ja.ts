...
const ja: Dict = {
  ...
  caption: {
    ...
    bag: "タグ袋",
    groups: "グループ",
    groupsHint: "タググループ段が削除するときのグループで、タグごとに色を付けます",
    groupNone: "グループなし",
    groupNoneHint: "Danbooru KB が知らないタグなので、どのグループでも削除されません",
    groupSolo: "このグループだけ表示 — もう一度押すと全部",
    groupsNoKb: "タググループには Danbooru タグ KB が要ります — 設定 › モデル から取得",
  },
  analysis: {
    badge: "分析",
    badgeHint:
  ...
  stage: {
    ...
    docs: {
      ...
      drop_groups:
        "revised キャプションから、タググループ(絵師、衣装、ライティングなど)をまとめて取り除きます。\n\n" +
        "切り取るだけです: 残ったタグは位置を保ち、何も足しません — バケット順も、トリガーワードも、@no-artist もありません。それは同じ削除のまわりで Correct がすることです。画風やコンセプトの LoRA で、削ったタグが代わりに説明していたものをトリガーワードに学ばせるためのステージです。Danbooru KB が知らないタグは決して削りません。読むのは revised キャプション、それが無い画像でだけ master で、master は編集しません。\n\n" +
        "既定はドライラン、--apply が書きます。続けて TE の再エンコードを回してください。",
      audit:
    ...
    notes: {
      ...
      drop_groups:
        "選んだグループを revised キャプションから取り除き、それ以外は動かしません。Danbooru KB が知らないタグは削りません。master は revised が無い画像でだけ読み、編集はしません。",
